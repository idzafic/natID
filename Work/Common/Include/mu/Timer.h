...
namespace mu
{
...
#ifdef MU_USETIMER
	#ifdef MU_WINDOWS
		typedef td::LINT8 TVar;
		typedef td::LINT8 TVar2;
	#else
		typedef td::LINT8 TVar;
        //typedef  std::chrono::time_point<std::chrono::steady_clock> TVar2;
        typedef  std::chrono::high_resolution_clock::time_point TVar2;
        //typedef std::chrono::high_resolution_clock TVar2;
	#endif
	...
	#else
			//TODO: implement using high resolution clock on other systems
			//return (TVar) clock();
    ...
    #else
            //return (1000.0 * _times[iClock].totalTime) / CLOCKS_PER_SEC;
    ...
    #else
            //return (double)_times[iClock].totalTime / CLOCKS_PER_SEC;
	...
	public:		
		...
		explicit Timer([[maybe_unused]] const char(&timerName)[size], [[maybe_unused]] bool startMeasuring = true)
		{
#ifdef MU_USETIMER
	#ifdef MU_WINDOWS
			LARGE_INTEGER tmp;
		...
		template <size_t size>
		void setName([[maybe_unused]] size_t iClock, [[maybe_unused]] const char(&clockName)[size], [[maybe_unused]] bool startMeasuring = true)
		{
		...
		inline void pause([[maybe_unused]] size_t iClock = 0)
		{
#ifdef MU_USETIMER
            assert(iClock <= SIZE);
		...
		inline void measure([[maybe_unused]] size_t iClock = 0)
		{
#ifdef MU_USETIMER
            assert(iClock <= SIZE);
			if (_times[iClock].status != 0)
		...
		inline void reset([[maybe_unused]] size_t iClock = 0)
		{
#ifdef MU_USETIMER
            assert(iClock <= SIZE);
	...
	#endif
			_times[iClock].lastStartTime = 0; //getCurrentTime();
#endif
		}
		...
		void showTimeInMillisec([[maybe_unused]] TSTREAM& s, [[maybe_unused]] size_t iClock = 0) const
		{
#ifdef MU_USETIMER
            assert(iClock <= SIZE);
...
//                o << iClock << ". NA: total=" << totalTime << "[ms], n=" << nCalls << ", avg=" << avgTime << "[ms]" << td::endl;
		...
		template <class TSTREAM>
		void showTimeInSec([[maybe_unused]] TSTREAM& s, [[maybe_unused]] size_t iClock = 0) const
		{
#ifdef MU_USETIMER
            assert(iClock <= SIZE);
			auto nCalls = _times[iClock].nCalls;
...
//                o << iClock << ". NA: total=" << totalTime << "[s], n=" << nCalls << ", avg=" << avgTime << "[s]" << td::endl;
		...
		inline bool isRunning([[maybe_unused]] size_t iClock = 0)
		{
#ifdef MU_USETIMER
            assert(iClock <= SIZE);

...
protected:
    ...
    bool onActionItem(gui::ActionItemDescriptor& aiDesc) override
    {
        ...
        if (menuID == 100)
        {
            showAlert("ViewComplex ctx menu", "TabView Header context menu option clicked!");
            return true;
        }
        return false;
    }
...
//    }
    ...
    bool onChangedSelection(gui::DBComboBox* pCmb) override
    {
        ...
        {
            td::String strKey;
            td::Variant keyVal(td::decimal3);
            if (pCmb->getValue(keyVal, true))
            {
                strKey = keyVal.toString();
            }

            int iSel = _dbCmb.getSelectedIndex();
            td::String selStr = _dbCmb.getSelectedText();
            td::String str;
            str.format("DBComboBox cs with key=%s:%d->%s\n", strKey.c_str(), iSel, selStr.c_str());
            _textEdit.appendString(str);
            td::Variant key;
            _dbCmb.getValue(key);
            _quant.setValue(key);

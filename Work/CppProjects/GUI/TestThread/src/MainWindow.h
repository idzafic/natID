...
protected:
    ...
    gui::AsyncFn _onThreadStart;
    gui::AsyncFn _onThreadStop;
    MainView _mainView;
protected:
    ...
    bool shouldClose() override
    {
        ...
        {
            td::String strHeader(tr("closeThNOK"));
            td::String strInfo(tr("stopThBefClose"));
            showAlert(strHeader, strInfo);
            return false;
        }
        return true;
    }
...
public:
    ...
    , _imgStop(":stop")
    , _toolBar(&_imgStart)
    , _onThreadStart(std::bind(&MainWindow::onThreadStart, this))
    , _onThreadStop(std::bind(&MainWindow::onThreadStop, this))
    , _mainView(&_onThreadStart, &_onThreadStop)
    {
        setTitle(tr("appTitle"));
        _mainMenuBar.setAsMain(this,gui::MenuBar::Location::EmbeddedInWindow);
        setToolBar(_toolBar);

...
const float FPS = 60.0f; //desired frame per second
...
public:
    ...
    , _fnSetVisualEffects(std::bind(&ViewTwin::setVisualEffects, &_animation))
    {
        setMargins(0, 0, 0, 0);
        _splitter.setContent(_animation, _properties);
        _animation.setSimulationParameters(&_simulationParams);
        _properties.setCallback(&_simulationParams, &_fnSetVisualEffects);
        setLayout(&_splitter);
    }
    ...
    void startStop()
    {
        ...
        {
            _timer.stop();
            _simulationParams.animation = false;
            _animation.stop();
            (*_pUpdateMenuAndTB)();
            _properties.enableControls();
        }
        else
        {
            _simulationParams.animation = true;
            _animation.start();
            _timer.start();
            (*_pUpdateMenuAndTB)();
            _properties.disableControls();
        }
    }

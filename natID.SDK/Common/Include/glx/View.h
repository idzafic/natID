...
namespace glx
{
enum class ImageFormat : td::BYTE { PNG = 0, BMP, TGA, JPG };
class ViewHelper;
...
private:
    ...
    glx::Device _device;
	std::mutex _currentDrawableMutex;
    FakeRenderPassData _fakeRenderPassData;
    
    glx::Size _pendingSize;     // Just mark that resize is needed (keep required size)
    bool _resizePending = false; // Don't recreate swapchain immediately during resize!
    bool _useFakeRenderPass = false;
private:

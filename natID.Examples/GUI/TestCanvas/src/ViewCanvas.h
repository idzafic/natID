...
//    }
    ...
    void onDraw(const gui::Rect& rect) override
    {
        ...
        //show drawable string taken from <Texts>
        _dsFromRes.draw({10, 670}, gui::Font::ID::SystemBold, td::ColorID::SysText);
...
public:
    ...
    , _symbolBez2(":Bez2")
    {
        //load a resource text from "<Texts>
        td::String strFromRes = gui::getResText(":txtSample");
        _dsFromRes = strFromRes;    //set it to drawable string
        ...
        //MNIST Image test
        {
            //Example usage of "FileNames" from resource
            //Resource file names can have path that starts with ~ which depicts HOME foldre (outside of natID.SDK)
            auto fn = gui::getResFileName(":mnist");
            ...
            if (nImages == 60000)
            {
                _mnist1.loadMNISTImage(f, 100);     //load image at position 100
                _mnist2.loadMNISTImage(f, 200);     //load image at positoin 200
            }
        }

...
protected:
    ...
    void initTable(int visPopulateType)
    {
        ...
        else
        {
            ...
            << gui::Header(1, tr("ProductN"), tr("ProductNTT"), 250)
            << gui::Header(2, tr("Quant"), tr("QuantTT"), 150, td::HAlignment::Right)
            << gui::Header(3, tr("Price"), tr("PriceTT"), 150, td::HAlignment::Right)
            << gui::Header(9, tr("Value"), tr("ValueTT"), 150, td::HAlignment::Right)
            ...
            << gui::Header(8, tr("Color"), tr("ColorTT"), 120);
            _table.init(_pDS);
        }
    ...
    void populateData()
    {
        td::String dbFileName = gui::getResFileName(":Test.db");
...
//            );
            ...
            float r4 = 15.72726;
            double r8 = 1024.7863;
            td::ColorID red = td::ColorID::Red;
            par << elemID << strVar << quant << price << date << time << r4 << r8 << red;

            dp::Transaction tr(_db.ptr());
            //delete some data
            dp::IStatementPtr pDel(_db->createStatement("DELETE FROM Test WHERE ID>4"));
            if (!pDel->execute())
                return;
            for (int i=0;i<100; ++i)
            {
                elemID += (i+1)*3;
                td::Decimal4 dec4(1.0555);
                price *= dec4;
                td::Decimal3 dec3(1.25);
                quant *= dec3;

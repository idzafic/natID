...
{
    static constexpr size_t INITIAL_BUFFER_SIZE = 10000000;
    ...
    InstancedObject(const VertexType* verts, size_t vertCount, const uint32_t* inds, size_t indCount, uint32_t maxInstances = 50);
    InstancedObject(const std::vector<VertexType> &verts, const std::vector<uint32_t> &inds, uint32_t maxInstances = 50);
    ...
    void draw(glx::RenderCommandEncoder& cmdEnc, uint32_t vertexBufferIndex, uint32_t instanceBufferIndex);
};
...
template<typename VertexType, typename InstanceDataType>
InstancedObject<VertexType, InstanceDataType>::InstancedObject(const VertexType* verts, size_t vertCount, const uint32_t* inds, size_t indCount, uint32_t maxInstances)
: _vertexOffset(0)
, _vertexCount(0)
, _indexOffset(0)
, _indexCount(0)
...
, _activeInstances(0)
{
    ...
    size_t indexDataSize = align4(_indexCount * sizeof(uint32_t));
...
template<typename VertexType, typename InstanceDataType>
InstancedObject<VertexType, InstanceDataType>::InstancedObject(const std::vector<VertexType> &verts, const std::vector<uint32_t> &inds, uint32_t maxInstances)
...
template<typename VertexType, typename InstanceDataType>
void InstancedObject<VertexType, InstanceDataType>::draw(glx::RenderCommandEncoder& cmdEnc, uint32_t vertexBufferIndex, uint32_t instanceBufferIndex)
{

...
class EditorView : public gui::ViewportCanvas
{
...
protected:
    ...
    {
        gui::ViewportCanvas::onPrimaryButtonPressed(inputDevice);
    ...
    void onPrimaryButtonReleased(const gui::InputDevice& inputDevice) override
    {
        ...
        gui::ViewportCanvas::onPrimaryButtonReleased(inputDevice);
    }
    ...
    {
        const auto& key = inputDevice.getKey();
        if (key.isShiftPressed())
        {
            gui::ViewportCanvas::onCursorDragged(inputDevice);
            return;
        }
...
//        setCursor(gui::Cursor::Type::Default);
        setClipsToBounds(gui::ClipMode::Right); //just for macOS
    }

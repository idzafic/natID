...
protected:
    ...
    td::MutableString _msFramePerSecond;
    //char _bufferFPF[16];
    //int _nCharsFPS = 0;
    ...
    void onDraw(const gui::Rect& rect) override
    {
        if (isAnimating())
        {
            ...
            if (dT > 0.5)
            {
                ...
                _msFramePerSecond.reset();  //do not destroy buffer, just reset it
                _msFramePerSecond.appendFormat("FPS:%.2f", FPS);
                _nFrames = 0;
            }
            else
            {
        ...
        if (_msFramePerSecond.length() > 0)
        {
            gui::Point pt(5,5);
            const auto& str = _msFramePerSecond.getString();
...
public:
    ...
    , _strInfo(tr("info"))
    {
        _msFramePerSecond.reserve(16); //16 chars is enough for FPS
        setPreferredFrameRateRange(60, 120);
